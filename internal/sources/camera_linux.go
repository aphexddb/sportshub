...
// listCamerasImpl enumerates real cameras on Linux. On a Raspberry Pi the /dev/video*
// space is dominated by ISP/codec/CSI pipeline nodes (rp1-cfe, pispbe, *-hevc-dec, fe_*),
// which are NOT usable cameras — the actual CSI sensor (e.g. an Arducam imx708) is owned by
// libcamera. So we list libcamera (CSI) cameras via rpicam, plus genuine USB/UVC capture
// devices, and skip the platform pipeline nodes entirely.
func listCamerasImpl() ([]Camera, error) {
	cams := append(libcameraCameras(), usbCameras()...)
	if len(cams) == 0 {
		return []Camera{{ID: "no-devices", Name: "No cameras found (CSI via libcamera or USB)"}}, nil
	}
	return cams, nil
}
...
// libcameraCameras parses `rpicam-hello --list-cameras` for CSI/libcamera cameras. Each is
// captured (in the ingest layer) via rpicam-vid, so its ID carries the "libcamera:" scheme.
...
// webcam exposes several video nodes (capture + metadata); we keep the first per USB interface.
func usbCameras() []Camera {
	...
	for _, dev := range devs {
		...
		seen[real] = true
		cams = append(cams, Camera{ID: dev, Name: readSysName(base)})
	}
	return cams
}
